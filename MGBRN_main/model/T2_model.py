...
class CFNm(nn.Module):
    def __init__(self, RT_model, TST_model, num_queries, device):
        ...
        self.RT_position_embedding = TSPositionEmbedding(self.embed_dim // 3, normalize=False, device=self.device)
        self.TST_position_embedding = TSPositionEmbedding(self.embed_dim // 3, normalize=False, device=self.device)
    ...
    def forward(self, images):
        ...
        RT_out, RT_pred = self.RT_model(images)         # b,t*196+1,d
        TST_out, TST_pred = self.TST_model(images)      # b,t*196+1,d


        RT_out = RT_out.permute(1, 0, 2)                                            # b,t*196+1,d -> t*196+1,b,d
        RT_mask = torch.zeros((b, RT_out.shape[0]), dtype=torch.bool).to(self.device)# 8,t*196+1
        RT_pos = self.RT_position_embedding(RT_mask)                                # t*196+1,b,d

        TST_out = TST_out.permute(1, 0, 2)                                          # b,t*196+1,d -> t*196+1,b,d
        hs = self.ffin(RT_out, TST_out, RT_mask, RT_pos)                            # t*196+1,8,768

        # 补充detr-decoder
        TST_mask = torch.zeros((b, hs.shape[0]), dtype=torch.bool).to(self.device)  # 8,t*196+1
        TST_pos = self.TST_position_embedding(TST_mask)                             # 8,768,1,t*196+1
