...
from transformers import TimesformerModel, TimesformerForVideoClassification, AutoModel
...
class TSPositionEmbedding(nn.Module):
    ...
    def forward(self, mask):                                                    # mask是全false矩阵
        ...
        not_mask = not_mask[:, 1:].reshape(b, 8, 14, 14)                        # b,8,14,14
...
class TSTwithoutFFIN(nn.Module):
    def __init__(self, num_frames, num_queries, device):
        super(TSTwithoutFFIN, self).__init__()
        self.frames = num_frames
        self.device = device
        self.embed_dim = 768
        self.num_action_class = num_queries

        self.backbone = TimesformerModel.from_pretrained("facebook/timesformer-base-finetuned-k400", num_frames=self.frames, ignore_mismatched_sizes=True, local_files_only=True)

        self.position_embedding = TSPositionEmbedding(self.embed_dim // 3, normalize=False, device=self.device)
        ...
        self.linear = nn.Linear(self.frames * 196 + 1, self.num_action_class)
        self.dim_linear = nn.Linear(self.backbone.config.hidden_size, self.embed_dim)
        self.action_group_linear = GroupWiseLinear(self.num_action_class, self.embed_dim, bias=True)

    def forward(self, images):
        x = self.backbone(images)[0]                # b,t,d

        # class-decoder部分
        #TST_mask = torch.zeros((x.shape[0], x.shape[1]), dtype=torch.bool).to(self.device)              # 8,1569
        #TST_pos = self.position_embedding(TST_mask)                                                 # 8,768,1,1569
        #pred_x = self.decoder(x.permute(1, 0, 2), self.query_embed.weight, TST_mask, TST_pos)[-1]       # 8,140,768

        # MLP部分
        pred_x = x.permute(0, 2, 1)
        pred_x = self.linear(pred_x)
        pred_x = pred_x.permute(0, 2, 1)
        pred_x = self.action_group_linear(pred_x)
        return x, pred_x

class TSTwithoutFFIN_NH(nn.Module):
    def __init__(self, num_frames, num_queries, device):
        super(TSTwithoutFFIN_NH, self).__init__()
        self.frames = num_frames
        self.device = device
        self.embed_dim = 256
        self.num_action_class = num_queries

        self.backbone = TimesformerModel.from_pretrained("facebook/timesformer-base-finetuned-k400", num_frames=self.frames, ignore_mismatched_sizes=True)
