...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, mask):
        # print(mask.shape)      # b,l
        not_mask = ~mask

        # 序列编码是一维的序列进行embed
        x_embed = not_mask.cumsum(1, dtype=torch.float32).to(self.device)
        # print(x_embed.shape)
        if self.normalize:
            # 防止除以0的操作
            eps = 1e-6
        ...
        # print(dim_t.shape)
        dim_t = self.temperature ** (2 * (dim_t // 2) / self.num_pos_feats)             # 768

        pos_x = x_embed[:, :, None] / dim_t                                             # 8,1577,768
        pos_x = torch.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3).flatten(2)
        pos = pos_x.permute(0, 2, 1)        # b,l,d -> b,d,l 8,1577,768 -> 8,768,1577
        return pos
...
class Decoder(nn.Module):
    ...
    def forward(self, memory, query_embed, mask, pos_embed):
        bs = memory.shape[1]
        pos_embed = pos_embed.permute(2, 0, 1)                  # t+1569,8,768
...
class CFN(nn.Module):
    def __init__(self, RT_model, TST_model, num_queries, device):
        ...
        self.position_embedding = PositionEmbeddingSine(self.embed_dim, normalize=False, device=self.device)

        self.linear = nn.Linear(8+1569, self.num_action_class)
        self.action_group_linear = GroupWiseLinear(self.num_action_class, self.embed_dim, bias=True)

    def forward(self, images):
        b, t, c, h, w = images.size()
        RT_out, RT_pred = self.RT_model(images)
        TST_out, TST_pred = self.TST_model(images)

        # concat的方法
        x = torch.cat((RT_out, TST_out), dim=1).permute(1, 0, 2)                    # t+1569,b,d

        # 补充detr-decoder
        #x_mask = torch.zeros((b, x.shape[0]), dtype=torch.bool).to(self.device)     # 8,t+1569
        #x_pos = self.position_embedding(x_mask)                                     # 8,768,t+1569
        #x = self.decoder(x, self.query_embed.weight, x_mask, x_pos)[-1]             # 8,140,768

        # MLP部分
        x = x.permute(1, 2, 0)      # b,d,t+1569
        x = self.linear(x)          # b,d,140
        x = x.permute(0, 2, 1)      # b,140,d
