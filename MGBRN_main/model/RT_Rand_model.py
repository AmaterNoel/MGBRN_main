...
class RT_Rand(nn.Module):
    def __init__(self, num_queries, device):
        super(RT_Rand, self).__init__()
        self.device = device
        self.embed_dim = 256
        ...
        self.encoder = Encoder(d_model=256, dropout=0.1, nhead=8, dim_feedforward=2048, normalize_before=True, return_intermediate_dec=True)
        self.ffin = FFIN(d_model=256, dropout=0.1, nhead=8, dim_feedforward=2048, normalize_before=True, return_intermediate_dec=True)
        self.query_embed = nn.Embedding(num_queries, self.encoder.d_model)
    ...
    def forward(self, images):
        ...
        memory = self.encoder(x, mask, pos)
        query_embed = self.query_embed.weight.unsqueeze(1).repeat(1, b, 1)
        hs = self.ffin(memory, query_embed, mask, pos)[-1]

        x = self.action_group_linear(hs)
        return x

class RT_Rand_NH(nn.Module):
    def __init__(self, num_queries, device):
        super(RT_Rand_NH, self).__init__()
        self.device = device
        self.embed_dim = 256
        self.num_action_class = num_queries
