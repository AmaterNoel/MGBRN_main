...
#from dataset import *
from Charades_dataset import *

parser = argparse.ArgumentParser(description='CFNm')
parser.add_argument("--seed", default=1, type=int, help="Seed for Numpy and PyTorch. Default: -1 (None)")
parser.add_argument("--device", default='cuda:4', type=str, help="0, 1, 2, 3, 4, 5, 6, 7")

# RT模型相关参数
parser.add_argument("--RT_type", default='withoutFFIN', type=str, help="withoutFFIN, Rand, CLIP")
parser.add_argument("--RT_pt", default='None', help="Load the trained model, Default: path or None")

# TST模型相关参数
parser.add_argument("--TST_type", default='withoutFFIN', type=str, help="withoutFFIN, Rand, CLIP")
parser.add_argument("--TST_pt", default='None', help="Load the trained model, Default: path or None")
...
parser.add_argument('--dataroot', default=r'path/Charades', type=str, help='dataset root')
opt = parser.parse_args()

# define dataset
action_class_list = get_dict()
action_class_list = list(action_class_list.keys())
print("num_action_class_list = ", len(action_class_list))

train_set = Charades(root=opt.dataroot, type='train', total_length=opt.frames, action_class_list=action_class_list)
...
test_set = Charades(root=opt.dataroot, type='test', total_length=opt.frames, action_class_list=action_class_list)
...
plt.legend()
plt.savefig('./eval_data/Charades_16' + str((start_epoch-1)*opt.total_epoch) + '-' + str(start_epoch*opt.total_epoch) + '.png')
